    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md -mt-16 p-8">
        <div className="flex items-center gap-4 relative">
          <ProfilePhoto userPic={formData.user_pic} setUserPic={setUserPic} editable={editMode} />
          <div>
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            <button
              className={`px-4 py-2 rounded-md text-white ${
                isComplete ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-300 cursor-not-allowed'
              }`}
              disabled={!isComplete}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
          {['phone', 'street', 'house_number', 'city', 'state', 'zip_code'].map((field) => (
            <ProfileInput
              key={field}
